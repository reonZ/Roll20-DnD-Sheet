...
const incomingCapture = /(?:characters|char-attribs\/char)\/(?<characterId>[^\/]+)(?:\/(?<attribId>[^\/]+$))?/

function onIncomingWsMessage(messageEvent, url, wsObject) {
    try {
        const data = JSON.parse(messageEvent.data)
        if (!data || !data.d || !data.d.b || !data.d.b.p || !('d' in data.d.b)) return messageEvent
        const result = incomingCapture.exec(data.d.b.p)
        if (!result) return messageEvent
        window.dispatchEvent(new CustomEvent('lvk-ws', { detail: { ...result.groups, data: data.d.b.d } }))
    } catch {}
    return messageEvent
}

window.wsHook.after = onIncomingWsMessage
